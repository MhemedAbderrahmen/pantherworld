...
const { status, data, error, refresh } = useFetch<MetaResponse | null>(
  `meta/`,
  {
...
<template>
  <section class="flex flex-col gap-3 justify-center items-center m-8 mb-16">
    ...
    <h1 class="text-4xl mb-6 text-center font-bold max-w-md">
  ...
  <section v-auto-animate>
    ...
    >
      <PlayerCard
        title="Latest claim"
        :player="data.meta.latestClaim.player"
        :item="data.meta.latestClaim.item"
      />
      <PlayerCard
        title="Latest cook"
        :player="data.meta.latestCook.player"
        :item="data.meta.latestCook.item"
      />
    </div>
    <div
      class="mb-16 grid grid-cols-1 sm:grid-cols-2 gap-8 justify-between items-start"
  ...
  <section v-auto-animate>
    <div
      class="mb-16 grid grid-cols-1 sm:grid-cols-3 gap-3 my-4"
      v-if="data !== null"
    >
      <div class="flex flex-col justify-between">
        <p
          class="font-bold uppercase text-lg mb-4 bg-catppuccin-green text-base text-center rounded-lg"
        >
          Latest Spawn
        </p>
        <InventoryItem
  ...
  <section class="mb-16" v-auto-animate>
    <div v-if="data !== null">
      <p
        class="font-bold uppercase text-xl bg-catppuccin-green text-base text-center rounded-lg"
      >
        Current Population
      </p>
