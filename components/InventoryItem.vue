...
<template>
  <div
    class="grid grid-cols-[2fr_5fr] grow gap-4 bg-base rounded-lg relative items-center p-4 bg-gradient-to-t via-10% to-zinc-800 to-60%"
    :class="{ 'from-lime-600 via-lime-800': irradiated }">
    <span
      v-if="displayCount"
      class="font-display absolute text-slate-50 bg-crust rounded-xl py1 px-2 -right-2 -top-2 font-bold"
      >{{ count }}x</span
    >
    <span
      v-if="extraInfo"
    ...
    <span class="justify-center w-[4rem] h-[4rem] p-4 rounded-full items-center flex bg-crust text-2xl leading-none">
